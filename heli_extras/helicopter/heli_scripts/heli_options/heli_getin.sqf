...
else
{
	...
	if (isNil "_lz") exitWith {
		if (alive _heli) then {
			player sideChat "Roger, no LZ marked, doing combat pickup at nearest possible point, Over";
			_heli land "GET OUT";
	...
	if (alive _heli) then
	{
		...
		player sideChat "Roger, We will be doing a combat pickup at the requested location, Over";
		while {alive _heli} do
		{
			...
			if ([_helipos select 0, _helipos select 1] distance _lz <= 100) exitWith {
				player sideChat "Commencing combat pickup, Over";
				_heli land "GET IN";
			};
		};
	};
