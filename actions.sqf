...
if (isDog) then {
	...
	lexisgreat = player addAction ["Bite", "noscript.sqf", ' if(!BiteCoolDown) then { 	systemChat "[Dog][Bite]: You must wait to Bite."; } else { 	BiteCoolDown = false; 	_men = nearestobjects [getpos player, ["CAManBase"], 2] - [player]; 	_man = _men select 0; 	player switchmove "Dog_StopV2"; 	if((isPlayer _man) and ((player distance _man) <= 2)) then { 		format[''%1 switchmove "AdthPpneMstpSlowWrflDf_1";'',_man] call broadcast; 		sleep M_punch; 		format["if (player == %2) then { [%1, %2, %3] spawn stun_hands_hit; };", player, _man, 20] call broadcast; 	} else { systemChat "[Dog][Bite]: It is to far away, or it is not a player!"; }; 	sleep 1; 	BiteCoolDown = true; }; '];
