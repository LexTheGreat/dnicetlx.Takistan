...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeS)) exitWith {
	...
	//// Wait until unload
	waitUntil {!isNil "UnloadCargo" || !(alive _Transporter) || !(alive _Object)};
	...
	if (DoDrop) then {
		...
		sleep 0.1;
		deTach _Object;
		_Object setPosATL [(getPosATL _Object select 0),(getPosATL _Object select 1),(getPosATL _Object select 2)-6];

		//// Create parachute and smoke
		sleep 2;
		_Parachute = createVehicle ["ParachuteBigWest", (getPosATL _Object), [], 0, "NONE"];
		_Parachute setPosATL (getPosATL _Object);
		_BlueSmoke = createVehicle ["SmokeShellBlue", (getPosATL _Object), [], 0, "NONE"];
		_BlueSmoke setPosATL (getPosATL _Object);
		...
		deTach _Object;
		sleep 3;
		_Object setPosATL [(getPosATL _Object select 0),(getPosATL _Object select 1),0.001];
